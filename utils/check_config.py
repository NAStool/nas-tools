...
def check_config(cfg):
    ...
    if config.get('app'):
        ...
        if not login_user or not login_password:
            log.error("【RUN】login_user或login_password未设置")
        else:
    ...
    if config.get('media'):
        ...
        if not movie_path:
            log.error("【RUN】未配置movie_path")
        elif not os.path.exists(movie_path):
            log.error("【RUN】movie_path目录不存在：%s" % movie_path)

        tv_path = config['media'].get('tv_path')
        if not tv_path:
            log.error("【RUN】未配置tv_path")
        elif not os.path.exists(tv_path):
            log.error("【RUN】tv_path目录不存在：%s" % tv_path)
