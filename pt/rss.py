...
class Rss:
                ...
                for res in rss_result:
                    try:
                        ...
                        if not media_info or not media_info.tmdb_info:
                            ...
                            continue
                        # 检查种子是否匹配订阅，返回匹配到的订阅ID、是否洗版
                        match_rssid, over_edition = Torrent().is_torrent_match_rss(media_info, movie_keys, tv_keys,
                                                                                   rss_job)
                            ...
                            for left_season in left_media:
                                if item.is_in_season(left_season.get("season")):
                                    if left_season.get("episodes"):
                                        ...
                                        update_rss_tv_lack(rssid=item.rssid, lack=len(left_season.get("episodes")))
                                        break
    ...
    def rsssearch_movie(self, rssid=None, state='D'):
        ...
        for movie in movies:
            ...
            # 已经存在
            if exist_flag:
                log.info("【RSS】电影 %s 已存在" % name)
                if not over_edition:
                    log.info("【RSS】删除电影订阅：%s" % name)
    ...
    def rsssearch_tv(self, rssid=None, state="D"):
        ...
        for tv in tvs:
            ...
            # 已经存在
            if exist_flag:
                log.info("【RSS】电视剧 %s%s 已存在" % (name, season))
                if not over_edition:
                    log.info("【RSS】删除电视剧订阅：%s%s" % (name, season))
            ...
            if not no_exists or not no_exists.get(media_info.get_title_string()):
                ...
                for no_exist_item in no_exist_items:
                    if str(no_exist_item.get("season")) == media_info.get_season_seq():
                        ...
                        break
                update_rss_tv_lack(rssid=rssid, lack=lack)
