...
class IIndexer(metaclass=ABCMeta):
    ...
    def __search(self, order_seq, indexer, key_word, filter_args: dict, match_type, match_words):
        ...
        if filter_args.get("site") and filter_args.get("site") != indexer_name:
            return []
        log.info(f"【{self.index_type}】开始检索Indexer：%s ..." % indexer_name)
        # 特殊符号处理
        ...
        if len(result_array) == 0:
            log.warn(f"【{self.index_type}】%s 未检索到资源" % indexer_name)
            return []
        else:
            log.warn(f"【{self.index_type}】%s 返回数据：%s" % (indexer_name, len(result_array)))
        # 从检索结果中匹配符合资源条件的记录
        index_sucess = 0
        for item in result_array:
            ...
            if match_type == 1 and not seeders:
                log.info(f"【{self.index_type}】%s 做种数为0，跳过..." % torrent_name)
                continue
            ...
            if match_type == 1:
                ...
                if not match_flag:
                    log.info(f"【{self.index_type}】%s 不符合过滤条件" % torrent_name)
                    continue
            else:
            ...
            if meta_info.type not in [MediaType.MOVIE, MediaType.UNKNOWN] and filter_args.get(
                ...
                log.info(f"【{self.index_type}】%s 是 %s，类型不匹配" % (torrent_name, meta_info.type.value))
                continue
            ...
            if filter_args.get("restype"):
                ...
                if restype_re and not re.search(r"%s" % restype_re, meta_info.resource_type, re.IGNORECASE):
                    log.info(f"【{self.index_type}】%s 不符合质量条件：%s" % (torrent_name, filter_args.get("restype")))
                    continue
            if filter_args.get("pix"):
                ...
                if restype_re and not re.search(r"%s" % restype_re, meta_info.resource_pix, re.IGNORECASE):
                    log.info(f"【{self.index_type}】】%s 不符合分辨率条件：%s" % (torrent_name, filter_args.get("pix")))
                    continue
            if filter_args.get("free") and not freeleech:
                log.info(f"【{self.index_type}】%s 不符合免费条件" % torrent_name)
                continue
            if filter_args.get("key") and not re.search(r"%s" % filter_args.get("key"), torrent_name, re.IGNORECASE):
                log.info(f"【{self.index_type}】%s 不符合关键字：%s" % (torrent_name, filter_args.get("key")))
                continue
            ...
            if match_type != 2:
                ...
                if not media_info or not media_info.tmdb_info:
                    log.info(f"【{self.index_type}】%s 未查询到媒体信息" % torrent_name)
                    continue

                # 类型
                if filter_args.get("type") and media_info.type != filter_args.get("type"):
                    log.info(f"【{self.index_type}】%s 是 %s，不匹配类型" % (torrent_name, media_info.type.value))
                    continue
                ...
                if match_type == 1:
                    ...
                    for match_word in match_words:
                        if str(match_word).upper() in str(media_info.get_title_string()).upper() \
                            ...
                            break
                    if not match_flag:
                        log.info(f"【{self.index_type}】%s：%s %s 不匹配名称：%s" % (
                ...
                if match_type == 1:
                    if not Torrent.is_torrent_match_size(media_info, self.__res_type, size):
                        log.info(f"【{self.index_type}】%s：%s %s 不符合大小要求" % (
            ...
            if not Torrent.is_torrent_match_sey(media_info, filter_args.get("season"), filter_args.get("episode"),
                ...
                log.info(f"【{self.index_type}】%s：%s %s 不匹配季/集/年份" % (
