    ...
    def __process_folder_subtitle(self, path):
        ...
        for file in self.__get_library_files(path):
            if not file:
                continue
            # 如果文件大小小于指定大小， 则不处理
            if os.path.getsize(file) < int(self.file_size):
                continue
            ...
            log.info(f"【Plugin】自动字幕生成 开始处理文件：{file}")
            # 导出音频到临时文件
            with tempfile.NamedTemporaryFile(suffix='.wav', delete=True) as audio_file:
                # 提取音频
                log.info(f"【Plugin】自动字幕生成 提取音频：{audio_file.name}")
                FfmpegHelper().extract_wav_from_video(file, audio_file.name)
                log.info(f"【Plugin】自动字幕生成 提取音频完成：{audio_file.name}")
                # 生成字幕
                ret, lang = self.__generate_subtitle(audio_file.name, audio_file.name)
                if not ret:
                    continue

                if self.translate_zh:
                    # 翻译字幕
                    log.info(f"【Plugin】自动字幕生成 翻译字幕：{audio_file.name}.srt")
                    self.__translate_zh_subtitle(f"{audio_file.name}.srt", f"{audio_file.name}.srt")
                    log.info(f"【Plugin】自动字幕生成 翻译字幕完成：{audio_file.name}.srt")
                    lang = 'zh'
                # 复制字幕文件
                log.info(f"【Plugin】自动字幕生成 复制字幕文件：{file_name}.{lang}.srt")
                SystemUtils.copy(f"{audio_file.name}.srt", f"{file_name}.{lang}.srt")
                # 删除临时文件
                os.remove(f"{audio_file.name}.srt")

    def __generate_subtitle(self, audio_file, subtitle_file):
        """
        生成字幕
        :param audio_file: 音频文件
        :param subtitle_file: 字幕文件
        :return: 生成成功返回True，字幕语言，否则返回False, None
        """
        # 生成字幕
        command = [self.whisper_main] + self.additional_args.split()
        command += ['-l', 'auto', '-m', self.whisper_model, '-osrt', '-of', subtitle_file, audio_file]
        ...
        if ret.returncode == 0:
            ...
            language = re.search(r"auto-detected language: (\w+)", output)
            if language and language.group(1):
                language = language.group(1)
            else:
                language = "en"
            log.info(f"【Plugin】自动字幕生成 生成字幕成功，原始语言：{language}")
