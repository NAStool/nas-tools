...
class DoubanApi(object):
    ...
    def movie_recommend(self, tags='', sort='U', start=0, count=20, ts=datetime.strftime(datetime.now(), '%Y%m%d')):
        return self.__invoke(self._urls["movie_recommend"], tags=tags, sort=sort, start=start, count=count, _ts=ts)

    def tv_recommend(self, tags='', sort='U', start=0, count=20, ts=datetime.strftime(datetime.now(), '%Y%m%d')):
