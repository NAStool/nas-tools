  ...
  __render_form_selectgroup(field_content) {
    ...
    for (let option in content) {
      ...
      if (this.config[id] && this.config[id].includes(option)) {
        checkbox = html`<input type="checkbox" name="${id}" value="${option}" onclick="${onclick}" class="form-selectgroup-input" checked>`
      } else {
        checkbox = html`<input type="checkbox" name="${id}" value="${option}" onclick="${onclick}" class="form-selectgroup-input">`
      }
